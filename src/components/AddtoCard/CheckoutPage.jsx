...
const handleConfirmOrder = async () => {
  ...
  const payload = {
    ...
    product_quantity: orderItems.reduce((sum, item) => sum + (parseInt(item.quantity) || 1), 0),
  ...
  try {
    const response = await fetch("https://backend.droploo.com/api/confirm-order", {
      method: "POST",
      headers: {
          ...
          <div className="space-y-2">
            ...
            <label className="flex items-center gap-2 p-3 border rounded hover:bg-gray-50">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600">Pay with credit/debit card or mobile banking</p>
              </div>
            </label>
          </div>
        </div>
