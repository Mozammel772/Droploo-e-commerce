...
//         ? item.selectedColor.color || "N/A" // Extract color name if it's an object
//         : item.selectedColor}
//     </p>
//   ) : (
...
//         ? item.selectedSize.size // Extract size value if it's an object
//         : item.selectedSize}
//     </p>
//   ) : (
...
//             <span>Subtotal</span>
//             <span className="text-base md:text-lg font-semibold">৳ {getTotalAmount().toFixed(2)}</span>
//           </div>
//           <div className="flex justify-between">
//             <span>Delivery Charge</span>
//             <span className="text-base font-bold">৳ {deliveryCharge.toFixed(2)}</span>
//           </div>
//           <div className="flex justify-between">
//             <span>Payment Method</span>
//             <span className="capitalize">{paymentMethod.toLowerCase()}</span>
//           </div>
//           <div className="flex justify-between font-bold text-lg md:text-xl border-t pt-2 mt-2">
//             <span>Total</span>
...
//           className={`mt-12 w-full px-6 py-3 rounded font-semibold ${
//             isSubmitting
...
const CheckoutPage = () => {
  ...
  const handleConfirmOrder = async () => {
    ...
    // Prepare products array with proper value handling
    const products = orderItems.map((item) => {
      // Handle color - convert object to string if needed
      let colorValue = "N/A";
      if (item.selectedColor) {
        colorValue = typeof item.selectedColor === 'object' 
          ? item.selectedColor.color || JSON.stringify(item.selectedColor)
      ...
      if (item.selectedSize) {
        sizeValue = typeof item.selectedSize === 'object'
          ? item.selectedSize.size || JSON.stringify(item.selectedSize)
    ...
    const payload = {
      ...
      delivery_area: deliveryCharge === 60 ? 1 : 2,
      customer_address: String(formData.address.trim()),
      product_quantity: orderItems.reduce(
        (sum, item) => sum + (Number(item.quantity) || 1),
        0
      ),
      price: Number((getTotalAmount() + deliveryCharge).toFixed(2)),
      payment_type: paymentMethod === "Cash On Delivery" ? "Cash" : "Online",
      order_type: "Online",
      customer_type: "Old Customer",
      employee_id: 1,
    ...
    try {
      ...
      Swal.fire({
        title: "Order Failed",
        html: `Error: ${error.message.replace(/\n/g, "<br>")}<br><br>Please check your order details and try again.`,
        icon: "error",
      });
    } finally {
      setIsSubmitting(false);
