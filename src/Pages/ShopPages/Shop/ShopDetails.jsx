...
const TABS = ["PRODUCT DETAILS", "RETURN POLICY", "REVIEW", "VIDEO", "TERMS & CONDITIONS"];
...
const TabContent = ({ tab, product }) => {
  switch (tab) {
    ...
    case "RETURN POLICY":
      return (
        <div className="bg-gray-100 p-4 rounded-md text-sm leading-relaxed">
          আগে পণ্য দেখে নিন, তারপর ডেলিভারি ম্যানকে টাকা দিন।<br />
          ব্যবহার করা পণ্য ফেরতযোগ্য নয়।<br />
          <strong>Call Us:</strong> +8801648177071<br />
          <strong>Email:</strong> banggomartbd@gmail.com
        </div>
      );
    case "REVIEW":
      return <p className="text-sm text-gray-600">রিভিউ সেকশন এখানে থাকবে (ডামি টেক্সট)।</p>;
    case "VIDEO":
    ...
    case "TERMS & CONDITIONS":
      return (
        <p className="text-sm text-gray-600">
          সকল পণ্য কোম্পানির নির্ধারিত নীতিমালার ভিত্তিতে সরবরাহ করা হয়। বিস্তারিত শর্তাবলি প্রযোজ্য।
        </p>
      );
    default:
      return null;
      ...
      <div className="flex flex-col md:flex-row gap-8">
        ...
        <div className="md:w-1/2 space-y-4">
          ...
          <div className="flex gap-4 mt-4">
            ...
            <button
              onClick={() => alert(`Purchased ${quantity} item(s).`)}
              className="flex-1 bg-orange-600 hover:bg-orange-700 text-white py-3 rounded font-semibold transition"
            >
              কিনে ফেলুন
            </button>
          </div>
         <div className="bg-white text-gray-800 rounded-md shadow-md p-4 border text-sm leading-relaxed space-y-2">
...
// const fetchProductDetails = async (slug) => {
//   const res = await fetch(`https://backend.droploo.com/api/product/details/${slug}`);
//   if (!res.ok) throw new Error("Failed to load product");
//   return res.json();
// };
...
//           <p className="text-xl text-teal-600 font-semibold mb-2">৳ {product?.regular_price}</p>
...
//           {product?.policy && (
//             <div
//               className="text-sm text-gray-700 border-t pt-4 mt-4"
//               dangerouslySetInnerHTML={{ __html: product.policy }}
//             />
//           )}
//         </div>
...
//           <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
//             {related.map((item) => (
//               <div key={item.id} className="border rounded p-2 bg-white shadow hover:shadow-md">
//                 <img
//                   src={item.imageUrl}
//                   alt={item.name}
//                   className="w-full h-40 object-cover rounded mb-2"
...
//                 <p className="text-teal-600 font-semibold text-sm mb-2">৳ {item.regular_price}</p>
//                 <a
//                   href={`/products-collection/details/${item.slug}`}
//                   className="block text-center bg-teal-600 text-white text-sm py-1 rounded hover:bg-teal-700"
//                 >
//                   View Details
//                 </a>
